...
const form = useForm({
    ...
    target: props.announcement.target,
    target_tenant_ids: props.announcement.target_tenant_ids || [],
    is_active: props.announcement.is_active,
    is_dismissible: props.announcement.is_dismissible,
    starts_at: props.announcement.starts_at ? props.announcement.starts_at.slice(0, 16) : '',
    ends_at: props.announcement.ends_at ? props.announcement.ends_at.slice(0, 16) : '',
})

const submit = () => {
    form.put(route('superadmin.announcements.update', props.announcement.id))
}
</script>

<template>
    <Head :title="`Modifier - ${announcement.title}`" />

    <SuperAdminLayout :title="`Modifier - ${announcement.title}`">
        <div class="max-w-2xl mx-auto space-y-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-400">{{ announcement.title }}</p>
                </div>
            </div>
