...
const sendMessage = () => {
    ...
    form.post(route('superadmin.support.message', props.ticket.id), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset()
            scrollToBottom()
...
const updateStatus = () => {
    statusForm.put(route('superadmin.support.status', props.ticket.id), {
        preserveScroll: true,
        onSuccess: () => {
            ...
            <div class="flex-1 flex overflow-hidden">
                        ...
                        >
                            ...
                            >
                                ...
                                <div :class="[
                                    ...
                                    message.is_admin ? 'text-primary-200' : 'text-gray-400'
                                ]">
                                    {{ message.created_at }}
                                    <svg
                                        v-if="message.is_read && message.is_admin"
                                        class="w-3 h-3"
                                        fill="currentColor"
                                        viewBox="0 0 20 20"
                                    >
                        ...
                        <form @submit.prevent="sendMessage" class="flex flex-col gap-2">
                            <div class="flex items-center gap-2">
                                <label class="flex items-center gap-2 text-sm text-gray-600">
                                    ...
                                    />
                                    Note interne (invisible au tenant)
                                </label>
                            </div>
                ...
                <div class="w-80 bg-white border-l overflow-y-auto hidden lg:block">
                    <div class="p-4 space-y-4">
                        <h3 class="font-semibold text-gray-900">Informations Tenant</h3>

                        <div v-if="ticket.tenant" class="space-y-3">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-primary-100 text-primary-600 rounded-full flex items-center justify-center font-semibold">
                                    {{ ticket.tenant.name?.charAt(0) || 'T' }}
                                </div>
                                <div>
                                    <div class="font-medium">{{ ticket.tenant.name }}</div>
